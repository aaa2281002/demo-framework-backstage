...
//            return r.fail().setMsg("用户角色缓存异常!");
    ...
    public ResponseResult findByParamList(SystemUserRole param) {
        ...
        List<SystemUserRole> list = systemUserRoleMapper.findByList(param);
        return rr.success().setData((list != null && list.size() > NumeralUtil.POSITIVE_ZERO) ? list.get(NumeralUtil.POSITIVE_ZERO) : null);
    }
     ...
     */
    @Override
    public ResponseResult findByRoleCodeList(String roleCode) {
        SystemRole systemRole = new SystemRole();
        systemRole.setRoleCode(roleCode);
        systemRole.setGtaeOperaterStatus(NumeralUtil.POSITIVE_ONE);
        systemRole = systemRoleMapper.selectByParam(systemRole);
        if (systemRole == null) {
            return super.getResponseResult().success();
        }
        SystemUserRole systemUserRole = new SystemUserRole();
        systemUserRole.setRoleId(systemRole.getId());
        List<SystemUserRole> list = systemUserRoleMapper.findByList(systemUserRole);
        return super.getResponseResult().success().setData(list);
    }
