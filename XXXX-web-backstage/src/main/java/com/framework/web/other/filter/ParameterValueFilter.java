...
public class ParameterValueFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    public ParameterValueFilter(PathMatcher pathMatcher, IgnoredUrlsProperties ignoredUrlsProperties) {
...
//        );
        boolean is = false;
        if (ignoredUrlsProperties.getParameterNotFilterUrl() != null
                && ignoredUrlsProperties.getParameterNotFilterUrl().size() > NumeralUtil.POSITIVE_ZERO) {
            for (String item : ignoredUrlsProperties.getParameterNotFilterUrl()) {
                is = pathMatcher.match(item, requestUri);
                if (is) {
                    break;
                }
            }
