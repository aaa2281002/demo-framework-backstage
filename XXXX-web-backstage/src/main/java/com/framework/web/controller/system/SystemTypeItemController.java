...
public class SystemTypeItemController extends BaseController {
    private String path = "lowerright/system/type/item/";
    @Autowired
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_MENU_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT')")
    public ModelAndView pageList(@NotNull(message = "请选择上级") @Min(value = NumeralUtil.POSITIVE_ONE, message = "上级不存在") Long parentId) {
        ModelAndView mv = new ModelAndView(path + "typeItemList");
        mv.addObject("parent", systemTypeServiceImpl.selectByPrimaryKey(parentId));
        return mv;
    }
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:add')")
    public ModelAndView getAdd(@NotNull(message = "请选择上级") @Min(value = NumeralUtil.POSITIVE_ONE, message = "上级不存在") Long parentId) {
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:edit')")
    public ModelAndView getEdit(@NotNull(message = "请选择类型") @Min(value = NumeralUtil.POSITIVE_ONE, message = "类型不存在") Long id,
        ...
        mv.addObject("parent", systemTypeServiceImpl.selectByPrimaryKey(parentId));
        mv.addObject("p", systemTypeServiceImpl.getByIdParam(id, parentId));
        return mv;
    }
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:view')")
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_MENU_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT')")
    @QueryTarget
    public ResponseResult findPageList(@Validated(value = {ValidationGroup.formQueryItem.class, ValidationGroup.formPageQuery.class}) SystemType param) {
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:add')")
    public ResponseResult save(@Validated(value = {ValidationGroup.formAddItem.class}) SystemType param, HttpServletResponse response) {
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:edit')")
    public ResponseResult edit(@Validated(value = {ValidationGroup.formEditItem.class}) SystemType param) {
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:batchDel')")
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:del')")
...
//    @ResponseBody
//    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_TYPE_ITEM_LIST_MANAGEMENT:isExist')")
