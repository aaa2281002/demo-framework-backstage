 ...
 * @Description 系统后台操作白名单IP请求控制类
 * @DateTime 2019/10/11
 * @Version 1.0
 */
@Controller
@RequestMapping(value = "/system/white/list/ip")
public class SystemWhiteListIpController extends BaseController {
    private String path = "lowerright/system/whiteListIp/";
    @Autowired
    private SystemWhiteListIpService systemWhiteListIpServiceImpl;
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_MENU_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT')")
    public ModelAndView pageList() {
        return new ModelAndView(path + "whiteListIpList");
    }
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT:add')")
    public ModelAndView getAdd() {
        ModelAndView mv = new ModelAndView(path + "whiteListIpAdd");
//        mv.addObject("p", systemWhiteListIpServiceImpl.selectByPrimaryKey(id));
        return mv;
    }
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT:edit')")
    public ModelAndView getEdit(Long id) {
        ModelAndView mv = new ModelAndView(path + "whiteListIpEdit");
        mv.addObject("p", systemWhiteListIpServiceImpl.getByIdParam(id));
        return mv;
    }
    ...
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT:view')")
    public ModelAndView getView(Long id) {
        ModelAndView mv = new ModelAndView(path + "whiteListIpView");
        mv.addObject("p", systemWhiteListIpServiceImpl.getByIdParam(id));
        return mv;
    }

    /**
     * @param param 1 系统后台操作白名单IP实体类
     * @return com.framework.common.response.ResponseResult
     * @Titel 系统后台操作白名单IP分页查询
     * @Description 系统后台操作白名单IP分页查询
     * @Author 邋遢龘鵺
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_MENU_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT')")
    public ResponseResult findPageList(SystemWhiteListIp param) {
        try {
            return systemWhiteListIpServiceImpl.findParamPageList(param);
        } catch (Exception e) {
            e.printStackTrace();
            return getError();
        }
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT:add')")
    public ResponseResult save(SystemWhiteListIp param, HttpServletResponse response) {
        try {
            return systemWhiteListIpServiceImpl.save(param);
        } catch (Exception e) {
            e.printStackTrace();
            return getError();
        }
    ...
    @ResponseBody
    @PreAuthorize("hasPermission('" + SystemUtil.SYSTEM_BUTTON_NAME + "','SYSTEM_WHITE_LIST_IP_MANAGEMENT:edit')")
    public ResponseResult edit(SystemWhiteListIp param) {
        try {
            return systemWhiteListIpServiceImpl.edit(param);
        } catch (Exception e) {
            e.printStackTrace();
            return getError();
        }
    ...
    /**
     * @param idList 1 系统后台操作白名单IP编号集合
     * @return com.framework.common.response.ResponseResult
     * @Titel 删除或批量删除
     * @Description 删除或批量删除
     * @Author 邋遢龘鵺
