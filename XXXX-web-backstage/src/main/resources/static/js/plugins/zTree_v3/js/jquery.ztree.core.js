        ...
        $.ajax({
          ...
          success: function (msg) {
            ...
            if(msg.code != '200' || msg.code != 200){
                ...
                if(msg.code == "-3" || msg.code == -3){
                    window.ALL.layerExtend.failMsg(msg.message,function(){
                        window.location = "/"
                    });
                    return ;
                }
                window.ALL.layerExtend.failMsg(msg.message);
                return ;
            }
            var newNodes = [];
            try {
              if (!msg || msg.length == 0) {
                ...
                newNodes = eval("(" + msg + ")");
              } else if(setting.async.dataType == "JSON" || setting.async.dataType == "json"){
                newNodes = msg.rows;
                sessionStorage.setItem("token", msg.token);
              } else {
                newNodes = msg;
