...
layui.define(["element", "jquery"], function (exports) {
    ...
    layuimini = new function () {
        ...
        this.init = function (url) {
            ...
            sessionStorage.setItem('layuiminiBgcolorId', 1);
            layuimini.initBgColor();
            layuimini.initDevice();
            $.getJSON(url, function (data, status) {
                // layuimini.msg_error('暂无菜单信息');
                // layuimini.msg_success('暂无菜单信息');
                if (data == null) {
                    layuimini.msg_error('暂无菜单信息');
                } else if(data.code != "0"){
                    layuimini.msg_error(data.message);
                }else {
        ...
        this.initHome = function (data) {
            sessionStorage.setItem('layuiminiHomeHref', data.urlPath);
            $('#layuiminiHomeTabId').html('<i class="' + data.icon + '"></i> <span>' + data.menuName + '</span>');
            $('#layuiminiHomeTabId').attr('lay-id', data.urlPath);
            $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + data.urlPath + '"></iframe>');
        };
        ...
        this.initMenu = function (data) {
            ...
            $.each(data, function (key, val) {
                headerMenuHtml += '<li class="layui-nav-item ' + headerMenuCheckDefault + '" id="' + key + 'HeaderId" data-menu="' + key + '"> <a href="javascript:;"><i class="' + val.icon + '"></i> ' + val.menuName + '</a> </li>\n';
                headerMobileMenuHtml += '<dd><a href="javascript:;" id="' + key + 'HeaderId" data-menu="' + key + '"><i class="' + val.icon + '"></i> ' + val.menuName + '</a></dd>\n';
                leftMenuHtml += '<ul class="layui-nav layui-nav-tree layui-left-nav-tree ' + leftMenuCheckDefault + '" id="' + key + '">\n';
                var menuList = val.childMenuList;
                $.each(menuList, function (index, menu) {
                    leftMenuHtml += '<li class="layui-nav-item">\n';
                    if (menu.childMenuList != undefined && menu.childMenuList.length > 0) {
                        leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips" ><i class="' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.menuName + '</span> </a>';
                        var buildChildHtml = function (html, childMenuList, menuParameId) {
                            html += '<dl class="layui-nav-child">\n';
                            $.each(childMenuList, function (childIndex, childMenu) {
                                html += '<dd>\n';
                                if (childMenu.childMenuList != undefined && childMenu.childMenuList.length > 0) {
                                    html += '<a href="javascript:;" class="layui-menu-tips" ><i class="' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.menuName + '</span></a>';
                                    html = buildChildHtml(html, childMenu.childMenuList, menuParameId);
                                } else {
                                    html += '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + childMenu.urlPath + '" target="' + childMenu.target + '"><i class="' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.menuName + '</span></a>\n';
                                    menuParameId++;
                        ...
                        leftMenuHtml = buildChildHtml(leftMenuHtml, menu.childMenuList, menuParameId);
                    } else {
                        leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  data-type="tabAdd" data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + menu.urlPath + '" target="' + menu.target + '"><i class="' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.menuName + '</span></a>\n';
                        menuParameId++;
                    }
                    leftMenuHtml += '</li>\n';
                });
