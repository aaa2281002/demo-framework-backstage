    ...
    var miniMenu = {
        ...
        renderChildrenMenu:function(menuList,options){
            ...
            var html = this.each(menuList,function (idx,menu) {
                if(menu.childMenuList && menu.childMenuList.length){
                    menu.children = me.renderChildrenMenu(menu.childMenuList,{ childOpenClass: options.childOpenClass || '' });
                }
                menu.className = "" ;
                menu.childOpenClass = options.childOpenClass || ''
                menu.title= menu.menuName;
                menu.href= menu.urlPath;
                menu.target= menu.target;
                menu.children= menu.childre;
        ...
        renderLeftMenu :function(leftMenus,options){
            ...
            var leftMenusHtml =  me.each(leftMenus || [],function (idx,leftMenu) { // 左侧菜单遍历
                var children = me.renderChildrenMenu(leftMenu.childMenuList, { childOpenClass:options.childOpenClass });
                var leftMenuHtml = me.compileMenu({
                    href:leftMenu.urlPath,
                    target:leftMenu.target,
                    className:"",
                    childOpenClass:options.childOpenClass,
                    icon:leftMenu.icon,
                    title:leftMenu.menuName,
                    children:children
                });
                return leftMenuHtml ;
            }).join("");
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            var headerMenuHtml = this.each(menuList, function (index, val) { //顶部菜单渲染
                ...
                leftMenuHtml+=me.renderLeftMenu(val.childMenuList,{
                    parentMenuId:menu,
                    childOpenClass:childOpenClass,
                    leftMenuCheckDefault:leftMenuCheckDefault
                });
                headerMobileMenuHtml +=me.compileMenu({ id:id,menu:menu,id:id,icon:val.icon, title:val.menuName, },true);
